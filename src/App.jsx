// ============================================================
// Macro Tracker Végétal — Application principale
...
// Sera modifiable dans le Bloc 5 (paramètres).
...
export default function App() {
  ...
  const [journal, setJournal] = useState(() => {
    try { const s = window.storage?.getItem(`journal:${dateAujourdhui()}`); if (s) return JSON.parse(s); } catch (e) {}
    const vide = {}; REPAS_DEFAUT.forEach((r) => { vide[r.id] = []; }); return vide;
  });

  useEffect(() => {
    try { window.storage?.setItem(`journal:${dateAujourdhui()}`, JSON.stringify(journal)); } catch (e) {}
  }, [journal]);
    ...
    <div className="min-h-screen bg-stone-50 text-stone-800">
      ...
      <header className="bg-emerald-700 text-white py-4 px-4 sticky top-0 z-20 shadow-md">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight">Macro Tracker Végétal</h1>
            <p className="text-emerald-200 text-xs">Suivi nutritionnel bienveillant</p>
          </div>
      ...
      <main className="max-w-xl mx-auto p-4 space-y-4">
        ...
        <p className="text-xs text-stone-400 text-center pb-4">
          Macro Tracker Végétal — Calculs basés sur Mifflin-St Jeor
        </p>
      </main>
    </div>
  );
